...
class AdminBot(commands.Cog, name="Moderation"):
    ...
    async def respond_to_deleted_message(self, message):
        ...
        async for entry in guild.audit_logs(limit=50, action=discord.AuditLogAction.message_delete):
            if entry.target.id == message.id and entry.user.id == ID_BOT_IGNORE:
                return
    ...
    async def wl_quotes(self, message):
        ...
        if (msg.startswith("!wl") or msg.startswith("!quote")) and message.channel.id != ID_WL_CHANNEL:
            wl_channel = message.guild.get_channel(ID_WL_CHANNEL)
            await message.channel.send(
                "{0.author.mention} pal, I think that stonks commands belongs in {1.mention} :chart_with_upwards_trend:"
                .format(message, wl_channel)
            )
    ...
    async def send_user_invite(self):
